...
#define FRONTIER_IMPL "../../../submissions/codingame/bots/rank_4_jacek_hybrid/submission.cpp"
#endif
...
static_assert(cg::kSafeHandoffFrontierWeight == 10,
              "The frozen frontier test is registered only for weight 10");
...
ps::GameState reconstruct_public_transcript(std::string_view transcript) {
...
void active_teacher_residual_frontier_cache_witness() {
  const ps::GameState state = reconstruct_public_transcript(
      "1/1/7/6/0/75/74/3/00523/135/01/13/27435/35");
  require(state.used_segments.size() >=
              static_cast<std::size_t>(cg::kTeacherResidualMinimumUsedEdges),
          "Teacher-residual witness lost the >=12-used-edge phase gate");
  const CandidateResult frontier = candidate_rebound_component(state);
  require(frontier.outcome == cg::ReboundOutcome::Unknown &&
              frontier.safe_endpoints > 0,
          "Teacher-residual witness lost its Unknown safe frontier");

  cg::SearchConfig residual_on;
  residual_on.max_nodes = 100;
  residual_on.max_time_ms = 0;
  residual_on.transposition_entries = 0;
  residual_on.evaluation_entries = 64;
  residual_on.exact_proof_mask = 0;
  residual_on.replay_value_blend_percent = 15;
  residual_on.teacher_residual_weight_percent = 100;
  cg::SearchStats residual_on_stats;
  const int unadjusted = leaf_score(state, residual_on, &residual_on_stats);
  require(residual_on_stats.teacher_residual_evaluations == 1,
          "Teacher residual did not activate on its phase-qualified witness");

  cg::SearchConfig residual_off = residual_on;
  residual_off.teacher_residual_weight_percent = 0;
  cg::SearchStats residual_off_stats;
  const int without_residual =
      leaf_score(state, residual_off, &residual_off_stats);
  require(residual_off_stats.teacher_residual_evaluations == 0 &&
              unadjusted != without_residual,
          "Teacher-residual witness did not produce a nonzero correction");

  cg::SearchConfig candidate = residual_on;
  candidate.exact_proof_mask = cg::kExactProofLeafBoundary;
  cg::CompleteTurnSearch search(state, candidate);
  const int first = search.leaf_score_for_test(7);
  const cg::SearchStats after_first = search.stats();
  const int second = search.leaf_score_for_test(7);
  const cg::SearchStats after_second = search.stats();
  const int sign = state.to_move == ps::Player::One ? 1 : -1;
  const int expected_delta =
      sign * static_cast<int>(frontier.safe_endpoints) *
      10 * (100 - candidate.replay_value_blend_percent) / 100;
  require(first == std::clamp(unadjusted + expected_delta,
                              -cg::kMaximumEvaluation,
                              cg::kMaximumEvaluation),
          "Active teacher residual changed the exact frontier delta");
  require(second == first &&
              after_first.teacher_residual_evaluations == 1 &&
              after_second.teacher_residual_evaluations == 1 &&
              after_first.leaf_rebound_probes == 1 &&
              after_second.leaf_rebound_probes == 1 &&
              after_second.evaluation_cache_hits ==
                  after_first.evaluation_cache_hits + 1 &&
              after_second.leaf_evaluations == after_first.leaf_evaluations,
          "Active residual frontier value was not cached after one proof");
}
...
int main() {
  ...
  constexpr std::array tests{
      ...
      Test{"active_teacher_residual_frontier_cache_witness",
           active_teacher_residual_frontier_cache_witness},
