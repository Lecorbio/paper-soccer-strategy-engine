...
if (report.schema !== "papersoccer.replay-value-model.v1") {
  throw new Error(`unsupported model schema: ${report.schema}`);
}
...
function floats(values) {
  return values.map((value) => Number(value).toPrecision(9) + "F").join(", ");
}
...
process.stdout.write(`Wrote ${outputPath} (${lines.join("\n").length} characters).\n`);
