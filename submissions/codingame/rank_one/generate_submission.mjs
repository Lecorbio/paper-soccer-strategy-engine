...
const manifestPath = path.join(scriptDirectory, "sources.txt");
const outputPath = path.join(scriptDirectory, "paper_soccer_rank_one.cpp");
...
if (argumentsList.length > 1 ||
  ...
  fail("Usage: node submissions/codingame/rank_one/generate_submission.mjs [--check]");
}
const checkOnly = argumentsList[0] === "--check";

const replayBookResult = spawnSync(
  process.execPath,
  [path.join(scriptDirectory, "generate_replay_book.mjs"),
   ...(checkOnly ? ["--check"] : [])],
  {encoding: "utf8"},
);
if (replayBookResult.status !== 0) {
  fail(replayBookResult.stderr || "Replay-book generation failed");
}
process.stdout.write(replayBookResult.stdout);

const replayValueResult = spawnSync(
  process.execPath,
  [path.join(scriptDirectory, "generate_replay_value_header.mjs"),
   ...(checkOnly ? ["--check"] : [])],
  {encoding: "utf8"},
);
if (replayValueResult.status !== 0) {
  fail(replayValueResult.stderr || "Replay-value generation failed");
}
process.stdout.write(replayValueResult.stdout);
...
const banner = [
  ...
  "// Rebuild with: node submissions/codingame/rank_one/generate_submission.mjs",
  "",
...
if (checkOnly) {
  if (!fs.existsSync(outputPath) || fs.readFileSync(outputPath, "utf8") !== output) {
    fail("paper_soccer_rank_one.cpp is stale; regenerate it.");
  }
