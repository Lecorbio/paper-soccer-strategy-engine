...
def load_samples(path):
    ...
    holdout_parts = [holdout_raw]
    for name in (
        "arena_loss_regressions.hpp",
        "observed_arena_loss_regressions.hpp",
        "training_arena_loss_regressions.hpp",
    ):
        contents = (ROOT / name).read_text()
        declared = re.search(r"std::array<Case,\s*(\d+)>\s+kCases", contents)
        cases = re.findall(
            r'\{\s*"(?:[^"\\]|\\.)*"\s*,\s*(\d+)\s*,\s*"([0-7/]*)"\s*\}',
            contents,
        )
        if declared is None or len(cases) != int(declared.group(1)):
            raise RuntimeError(f"could not parse every holdout in {name}")
        held_out_states.update((int(player), prefix) for player, prefix in cases)
        holdout_parts.append(contents.encode())
    ...
    return (
        ...
        rejected,
        corpus_hash,
        hashlib.sha256(b"".join(holdout_parts)).hexdigest(),
    )
...
def main():
    ...
    output_path = pathlib.Path(sys.argv[2]) if len(sys.argv) == 3 else ROOT / "rank5_residual_model.json"
