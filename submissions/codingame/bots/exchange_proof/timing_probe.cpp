...
double probe_exchange_position(std::string_view transcript) {
  ps::RulesConfig rules;
  rules.goal_rule = ps::GoalRule::OwnGoalsAllowed;
  rules.blocked_rule = ps::BlockedRule::MoverLoses;
  ps::GameState state = ps::make_initial_state(rules);
  ...
  if (ps::is_terminal(state)) {
    throw std::logic_error("timing exchange position is terminal");
  }
...
int main() {
  ...
  for (const auto &[name, transcript] : kExchangeCases) {
    std::cout << "shell=" << name
              << " later_ms=" << probe_exchange_position(transcript) << '\n';
  }
  return 0;
}
