...
namespace papersoccer::exchange_proof_engine {
...
#define turn_action_v2 exchange_proof_engine
#include "bot.cpp"
#undef turn_action_v2
...
namespace candidate = papersoccer::exchange_proof_engine;
...
Decision choose_incumbent(ps::GameState &state, std::string_view transcript,
  ...
  if (decision.nodes > node_budget) {
    throw std::logic_error("incumbent exceeded the node safety cap");
  }
...
Decision choose_candidate(ps::GameState &state, std::string_view transcript,
  ...
  if (!candidate::try_replay_correction(
    ...
    decision.exchange_ply1_win_hits = search.stats().exchange_ply1_win_hits;
    decision.exchange_ply1_loss_hits = search.stats().exchange_ply1_loss_hits;
    ...
    decision.exchange_ply2_win_hits = search.stats().exchange_ply2_win_hits;
    decision.exchange_ply2_loss_hits = search.stats().exchange_ply2_loss_hits;
    decision.exchange_ply2_cutoffs = search.stats().exchange_ply2_cutoffs;
  }
  if (decision.nodes > node_budget) {
    throw std::logic_error("candidate exceeded the node safety cap");
  }
  verify_complete_action(before, state, decision.action);
  return decision;
}
...
GameResult play(const Opening &opening, int candidate_player,
  ...
  while (!ps::is_terminal(state) && result.turns < maximum_turns) {
    ...
    if (candidate_turn) {
      ...
      result.exchange_ply1_probes += decision.exchange_ply1_probes;
      result.exchange_ply1_win_hits += decision.exchange_ply1_win_hits;
      result.exchange_ply1_loss_hits += decision.exchange_ply1_loss_hits;
      result.exchange_ply1_cutoffs += decision.exchange_ply1_cutoffs;
      result.exchange_ply2_probes += decision.exchange_ply2_probes;
      result.exchange_ply2_win_hits += decision.exchange_ply2_win_hits;
      result.exchange_ply2_loss_hits += decision.exchange_ply2_loss_hits;
      result.exchange_ply2_cutoffs += decision.exchange_ply2_cutoffs;
    } else {
...
void write_game(std::ostream &out, const GameResult &game) {
  ...
  out << ",\"turns\":" << game.turns
      ...
      << ",\"exchange_ply1_probes\":" << game.exchange_ply1_probes
      << ",\"exchange_ply1_win_hits\":" << game.exchange_ply1_win_hits
      << ",\"exchange_ply1_loss_hits\":" << game.exchange_ply1_loss_hits
      << ",\"exchange_ply1_cutoffs\":" << game.exchange_ply1_cutoffs
      << ",\"exchange_ply2_probes\":" << game.exchange_ply2_probes
      << ",\"exchange_ply2_win_hits\":" << game.exchange_ply2_win_hits
      << ",\"exchange_ply2_loss_hits\":" << game.exchange_ply2_loss_hits
      << ",\"exchange_ply2_cutoffs\":" << game.exchange_ply2_cutoffs
      << '}';
}
