...
void rebound_goal_is_proven_at_depth_zero() {
  ...
  require(rotated_moves.size() == 1 &&
              cg::encode_direction(rotated.ball, rotated_moves.front().to) == '0',
  ...
  require(rotated_search.stats().completed_turn_depth == 1 &&
          ...
          "The rotated attacking-shell proof must have the opposite score.");
}

void depth_zero_proof_is_skipped_outside_attacking_shell() {
  cg::SearchConfig config;
  config.max_turn_depth = 1;
  config.max_nodes = 1'000;
  config.max_time_ms = 0;
  ...
  ps::GameState south = make_clean_state_at({4, 6}, ps::Player::One);
  block_edges_except(south, {4, 6}, {{4, 7}});
  for (int y = 8; y <= 11; ++y) {
    south.visit_count[{4, y}] = 1;
  }
  cg::CompleteTurnSearch south_search(south, config);
  (void)south_search.run();
  require(south_search.stats().completed_turn_depth == 1 &&
              south_search.stats().rebound_goal_probes == 1 &&
              south_search.stats().rebound_goal_hits == 0 &&
              south_search.stats().root_score > -900'000 &&
              south_search.stats().root_score < 900'000,
          "A Player Two leaf outside its attacking shell must use evaluation.");

  ps::GameState north = make_clean_state_at({4, 6}, ps::Player::Two);
  block_edges_except(north, {4, 6}, {{4, 5}});
  for (int y = 1; y <= 4; ++y) {
    north.visit_count[{4, y}] = 1;
  }
  cg::CompleteTurnSearch north_search(north, config);
  (void)north_search.run();
  require(north_search.stats().completed_turn_depth == 1 &&
              north_search.stats().rebound_goal_probes == 1 &&
              north_search.stats().rebound_goal_hits == 0 &&
              north_search.stats().root_score > -900'000 &&
              north_search.stats().root_score < 900'000,
          "A Player One leaf outside its attacking shell must use evaluation.");
}
...
int main() {
  ...
  const std::vector<TestCase> tests{
      ...
      {"rebound_goal_is_proven_at_depth_zero",
       rebound_goal_is_proven_at_depth_zero},
      {"depth_zero_proof_is_skipped_outside_attacking_shell",
       depth_zero_proof_is_skipped_outside_attacking_shell},
