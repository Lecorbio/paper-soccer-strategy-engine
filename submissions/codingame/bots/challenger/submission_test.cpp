...
std::pair<std::string, cg::SearchStats> search_with_one_node(
  ...
  cg::apply_encoded_turn(verified, encoded);
  require(same_state(replayed, verified),
          "A rebound-goal probe action should replay exactly.");
  return {encoded, search.stats()};
}

void rebound_goal_reachability_is_exact() {
  ...
  ps::GameState south =
      make_clean_state_at(south_root, ps::Player::Two);
  ...
  ps::GameState own_goal =
      make_clean_state_at(north_root, ps::Player::Two);
...
void candidate_winning_jacek_branches_activate_exactly() {
  constexpr std::array<std::pair<std::string_view, std::string_view>, 2>
      branches{{
          {"0/0/1/6", "7"},
          {"0/0/3/67/27/45/5/2/5/6143/5/717271/1/7/5330", "2722"},
      }};
  for (const auto &[prefix, expected_action] : branches) {
    ps::GameState actual = reconstruct(prefix);
    ps::GameState expected = actual;
    cg::apply_encoded_turn(expected, expected_action);
    const int player_id =
        actual.to_move == ps::Player::One ? 0 : 1;
    std::string encoded = "sentinel";
    require(cg::try_replay_correction(actual, player_id, prefix, encoded),
            "A proven candidate-v1 jacek branch should activate.");
    require(encoded == expected_action && same_state(actual, expected),
            "A proven jacek branch should replay the winning action exactly.");
  }
}

void stable_horizon_branches_activate_exactly() {
  constexpr std::array<std::pair<std::string_view, std::string_view>, 8>
      branches{{
          {"6/7/5", "61"},
          {"6/7/5/61/44/53/0", "0617271"},
          {"6/7/5/61/44/53/0/0617271/3", "42"},
          {"0/0/0/6/6/5/2/5/0527271", "614347"},
          {"0/5/21/2/7/43/01/35/70/563/07/2505/7/4", "5"},
          {"0/0/3/67/27/45/5", "2"},
          {"7/6/7/53/10/34/71", "45"},
          {"0/6/5/5/71/34/1/033/5/5/2/3/31/4167", "2"},
      }};
  for (const auto &[prefix, expected_action] : branches) {
    ps::GameState actual = reconstruct(prefix);
    ps::GameState expected = actual;
    cg::apply_encoded_turn(expected, expected_action);
    const int player_id = actual.to_move == ps::Player::One ? 0 : 1;
    std::string encoded = "sentinel";
    require(cg::try_replay_correction(actual, player_id, prefix, encoded),
            "An evidence-backed horizon response should activate.");
    require(encoded == expected_action && same_state(actual, expected),
            "A horizon response should replay its winning action exactly.");
  }
}
...
int main() {
  ...
  const std::vector<TestCase> tests{
      ...
      {"interrupted_search_preserves_a_complete_action",
       interrupted_search_preserves_a_complete_action},
      {"rebound_goal_reachability_is_exact",
       rebound_goal_reachability_is_exact},
      {"selected_replay_correction_activates_exactly",
       selected_replay_correction_activates_exactly},
      {"candidate_winning_jacek_branches_activate_exactly",
       candidate_winning_jacek_branches_activate_exactly},
      {"stable_horizon_branches_activate_exactly",
       stable_horizon_branches_activate_exactly},
