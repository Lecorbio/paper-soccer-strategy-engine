...
void compare_position(std::string_view transcript, std::string_view label,
  ...
  std::cout << "position=" << label << " ball=" << opening.state.ball.x << ','
            ...
            << (reference_uses_v2 ? "selfplay_nn_v2:" : "rank_5:")
            << reference.action << ':'
...
int main(int argc, char **argv) {
  if (const char *value = std::getenv("SELFPLAY_V2_RESIDUAL_WEIGHT")) {
    candidate_residual_weight = std::atoi(value);
  }
  if (const char *value =
          std::getenv("SELFPLAY_V2_REFERENCE_RESIDUAL_WEIGHT")) {
    reference_residual_weight = std::atoi(value);
  }
  if (const char *value = std::getenv("SELFPLAY_V2_GAME_TIME_MS")) {
  ...
  if (const char *value = std::getenv("SELFPLAY_V2_CANDIDATE_FIRST_MS")) {
    candidate_first_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("SELFPLAY_V2_CANDIDATE_LATER_MS")) {
    candidate_later_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("SELFPLAY_V2_REFERENCE_FIRST_MS")) {
    reference_first_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("SELFPLAY_V2_REFERENCE_LATER_MS")) {
    reference_later_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("SELFPLAY_V2_REFERENCE_ENGINE")) {
    reference_uses_v2 = std::atoi(value) != 0;
  }
  if (const char *value = std::getenv("SELFPLAY_V2_REFERENCE_REPLAY")) {
    reference_replay_corrections = std::atoi(value) != 0;
  }
  if (const char *value = std::getenv("SELFPLAY_V2_CANDIDATE_REPLAY")) {
    candidate_replay_corrections = std::atoi(value) != 0;
  }
  if (const char *value = std::getenv("SELFPLAY_V2_MAX_DEPTH")) {
    candidate_max_depth = static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value =
          std::getenv("SELFPLAY_V2_REFERENCE_MAX_DEPTH")) {
