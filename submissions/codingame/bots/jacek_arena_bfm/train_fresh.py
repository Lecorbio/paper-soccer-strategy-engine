...
def load_data(
    ...
    *,
    ...
    validator = FreshCorpusValidator(
        ...
        excluded_game_ids=(
            load_excluded_game_ids(exclusions) if exclusions is not None else ()
        ),
        training_only=True,
    )
    ...
    for raw_path in paths:
        ...
        for raw in iter_jsonl(path):
            ...
            split = game_bucket(item.game_id)
            if item.kind == "value":
                values.append(
                    (
...
def value_batch(
    network: Network, x: np.ndarray, target: np.ndarray, weight: np.ndarray
...
def pair_batch(
    network: Network, preferred: np.ndarray, inferior: np.ndarray, weight: np.ndarray
) -> tuple[float, list[np.ndarray]]:
...
def train(
    ...
    value_weights = scale_arena_weights(data.value_w, data.value_arena, arena_exposure)
    pair_weights = scale_arena_weights(data.pair_w, data.pair_arena, arena_exposure) if len(data.pair_w) else data.pair_w
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(value_train), batch_size):
            index = value_train[start:start + batch_size]
            loss, gradients = value_batch(
                network, data.value_x[index], data.value_y[index], value_weights[index]
            )
            optimizer.step(gradients)
            losses.append(loss)
        for start in range(0, len(pair_train), batch_size):
            index = pair_train[start:start + batch_size]
            loss, gradients = pair_batch(
                network, data.pair_preferred[index], data.pair_inferior[index], pair_weights[index]
            )
            optimizer.step(gradients)
            losses.append(loss)
        history.append(
...
def main() -> int:
    ...
    parser.add_argument("--arena-exposure", type=float, default=0.0)
    ...
    if not 0.0 <= args.arena_exposure <= 1.0:
        parser.error("arena exposure must be in [0,1]")
    data = load_data(
        args.corpus,
    ...
    np.savez_compressed(args.output_model, w1=network.w1, w2=network.w2, w3=network.w3)
    manifest = {
        ...
        "whole_game_split": "sha256(game_id)[0] % 10: 0 validation, 1 scratch-test, 2-9 train",
        "history": history,
        "validation": metrics(network, data, 0),
