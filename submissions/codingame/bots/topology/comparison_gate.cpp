...
namespace papersoccer::topology_engine {
namespace replay_value_model =
    ::papersoccer::rank5_engine::replay_value_model;
}
#endif

#define turn_action_v2 topology_engine
#include "bot.cpp"
#undef turn_action_v2
...
namespace candidate = papersoccer::topology_engine;
...
struct Decision {
  ...
  int root_score{};
  double milliseconds{};
  std::uint64_t corridor_probes{};
  std::uint64_t corridor_activations{};
  std::uint64_t corridor_breakers{};
};
...
Decision choose_candidate(ps::GameState &state, std::string_view transcript,
  ...
  if (!candidate::try_replay_correction(
    ...
    decision.completed_depth = search.stats().completed_turn_depth;
    decision.root_score = search.stats().root_score;
    decision.corridor_probes = search.stats().corridor_ordering_probes;
    decision.corridor_activations =
        search.stats().corridor_ordering_activations;
    decision.corridor_breakers = search.stats().corridor_breaking_moves;
  }
  verify_complete_action(before, state, decision.action);
  return decision;
}
...
std::vector<Opening> read_bank(const std::string &path) {
  ...
  while (std::getline(input, line)) {
    ...
    opening.observed_action = fields[18];
    opening.transcript = fields[19] == "-" ? std::string{} : std::string(fields[19]);
...
GameResult play(const Opening &opening, int candidate_player,
  ...
  while (!ps::is_terminal(state) && result.turns < maximum_turns) {
    ...
    if (candidate_turn) {
      ...
      result.corridor_probes += decision.corridor_probes;
      result.corridor_activations += decision.corridor_activations;
      result.corridor_breakers += decision.corridor_breakers;
    } else {
...
void write_game(std::ostream &out, const GameResult &game) {
  ...
  out << ",\"turns\":" << game.turns
      ...
      << ",\"corridor_probes\":" << game.corridor_probes
      << ",\"corridor_activations\":" << game.corridor_activations
      << ",\"corridor_breakers\":" << game.corridor_breakers << '}';
}
