...
SearchProbe probe_search(const ps::GameState &state,
  ...
  cg::apply_encoded_turn(verified, action);
  require(same_state(replayed, verified),
          "A corridor-ordered action should remain complete and replayable.");
  return {action, search.stats()};
}
...
void corridor_ordering_is_goal_shell_selective_and_symmetric() {
  const SearchProbe center = probe_search(
      ps::make_initial_state(codingame_rules()));
  require(center.stats.corridor_ordering_activations == 0,
          "The initial midfield position must not activate goal-shell ordering.");

  const SearchProbe south = probe_search(
      make_clean_state_at({4, 10}, ps::Player::One));
  const SearchProbe north = probe_search(
      make_clean_state_at({4, 2}, ps::Player::Two));
  require(south.stats.corridor_ordering_probes > 0 &&
              north.stats.corridor_ordering_probes > 0 &&
              south.stats.corridor_ordering_activations > 0 &&
              north.stats.corridor_ordering_activations > 0,
          "Both rotated own-goal shells should detect the opponent corridor.");
  require(south.stats.corridor_breaking_moves > 0 &&
              north.stats.corridor_breaking_moves > 0,
          "Both rotated shells should contain a path-breaking edge.");
  require(south.action.size() == north.action.size(),
          "Rotated shell actions should have equal length.");
  for (std::size_t index = 0; index < south.action.size(); ++index) {
    const int rotated = (south.action[index] - '0' + 4) % 8;
    require(north.action[index] == static_cast<char>('0' + rotated),
            "Goal-shell ordering should be invariant under player rotation.");
  }
}

void frozen_elite_corridors_activate_without_mutating_input() {
  constexpr std::array<std::string_view, 3> threats{{
      "0/6/5/4/5/53/61/0633",
      "0/6/5/4/5/53/61/431",
      "1/1/7/6/0/75/74/3/003",
  }};
  std::uint64_t activations = 0;
  for (const std::string_view transcript : threats) {
    const ps::GameState state = reconstruct(transcript);
    const ps::GameState before = state;
    const SearchProbe probe = probe_search(state, 5'000);
    activations += probe.stats.corridor_ordering_activations;
    require(same_state(state, before),
            "A frozen elite probe must not mutate its input state.");
  }
  require(activations > 0,
          "At least one frozen elite threat must exercise corridor ordering.");
}
...
int main() {
  ...
  const std::vector<TestCase> tests{
      ...
      {"interrupted_search_preserves_a_complete_action",
       interrupted_search_preserves_a_complete_action},
      {"corridor_ordering_is_goal_shell_selective_and_symmetric",
       corridor_ordering_is_goal_shell_selective_and_symmetric},
      {"frozen_elite_corridors_activate_without_mutating_input",
       frozen_elite_corridors_activate_without_mutating_input},
