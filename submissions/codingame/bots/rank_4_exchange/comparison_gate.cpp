...
namespace regressions = papersoccer::rank_4_exchange_regressions;
namespace observed = papersoccer::observed_rank_4_exchange_regressions;

namespace {

int candidate_residual_weight = 100;
int reference_residual_weight = 100;
...
struct Decision {
  ...
  std::uint64_t rebound_probes{};
  std::uint64_t rebound_wins{};
  std::uint64_t rebound_losses{};
...
struct GameResult {
  ...
  std::uint64_t candidate_rebound_probes{};
  std::uint64_t candidate_rebound_wins{};
  std::uint64_t candidate_rebound_losses{};
  std::uint64_t candidate_exchange_ply1_probes{};
  std::uint64_t candidate_exchange_ply1_cutoffs{};
  std::uint64_t candidate_exchange_ply2_probes{};
  std::uint64_t candidate_exchange_ply2_cutoffs{};
...
Decision choose_reference_engine(ps::GameState &state,
  ...
  if (!replay_corrections ||
    ...
    config.teacher_residual_weight_percent = reference_residual_weight;
    if (reference_max_depth != 0) {
      config.max_turn_depth = reference_max_depth;
    }
...
Decision choose_candidate(ps::GameState &state, std::string_view transcript,
  ...
  if (!replay_corrections ||
    ...
    decision.residual_evaluations =
        search.stats().teacher_residual_evaluations;
    decision.rebound_probes = search.stats().rebound_goal_probes;
    decision.rebound_wins = search.stats().rebound_goal_hits;
    decision.rebound_losses = search.stats().rebound_loss_hits;
    decision.exchange_ply1_probes = search.stats().exchange_ply1_probes;
    decision.exchange_ply1_cutoffs = search.stats().exchange_ply1_cutoffs;
    decision.exchange_ply2_probes = search.stats().exchange_ply2_probes;
    decision.exchange_ply2_cutoffs = search.stats().exchange_ply2_cutoffs;
    decision.budget_exhausted = search.stats().budget_exhausted;
    decision.searched = true;
  }
  verify_complete_action(before, state, decision.action);
...
Decision choose_reference(ps::GameState &state, std::string_view transcript,
  ...
  return choose_reference_engine(state, transcript, node_budget,
...
GameResult play(const Opening &opening, int candidate_player,
  ...
  while (!ps::is_terminal(state) && result.turns < maximum_turns) {
    ...
    if (candidate_turn) {
      ...
      result.candidate_rebound_probes += decision.rebound_probes;
      result.candidate_rebound_wins += decision.rebound_wins;
      result.candidate_rebound_losses += decision.rebound_losses;
      result.candidate_exchange_ply1_probes +=
          decision.exchange_ply1_probes;
      result.candidate_exchange_ply1_cutoffs +=
          decision.exchange_ply1_cutoffs;
      result.candidate_exchange_ply2_probes +=
          decision.exchange_ply2_probes;
      result.candidate_exchange_ply2_cutoffs +=
          decision.exchange_ply2_cutoffs;
...
void record_pair(const Opening &opening, std::uint64_t node_budget,
  ...
  for (int candidate_player = 0; candidate_player < 2; ++candidate_player) {
    ...
    summary.candidate_rebound_probes += game.candidate_rebound_probes;
    summary.candidate_rebound_wins += game.candidate_rebound_wins;
    summary.candidate_rebound_losses += game.candidate_rebound_losses;
    summary.candidate_exchange_ply1_probes +=
        game.candidate_exchange_ply1_probes;
    summary.candidate_exchange_ply1_cutoffs +=
        game.candidate_exchange_ply1_cutoffs;
    summary.candidate_exchange_ply2_probes +=
        game.candidate_exchange_ply2_probes;
    summary.candidate_exchange_ply2_cutoffs +=
        game.candidate_exchange_ply2_cutoffs;
...
void compare_position(std::string_view transcript, std::string_view label,
  ...
  const Decision reference = choose_reference_engine(
      reference_state, transcript, node_budget,
...
int main(int argc, char **argv) {
  if (const char *value = std::getenv("RANK_4_EXCHANGE_RESIDUAL_WEIGHT")) {
    candidate_residual_weight = std::atoi(value);
  }
  if (const char *value =
          std::getenv("RANK_4_EXCHANGE_REFERENCE_RESIDUAL_WEIGHT")) {
    reference_residual_weight = std::atoi(value);
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_GAME_TIME_MS")) {
  ...
  if (const char *value = std::getenv("RANK_4_EXCHANGE_CANDIDATE_FIRST_MS")) {
    candidate_first_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_CANDIDATE_LATER_MS")) {
    candidate_later_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_REFERENCE_FIRST_MS")) {
    reference_first_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_REFERENCE_LATER_MS")) {
    reference_later_time_ms =
        static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_REFERENCE_REPLAY")) {
    reference_replay_corrections = std::atoi(value) != 0;
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_CANDIDATE_REPLAY")) {
    candidate_replay_corrections = std::atoi(value) != 0;
  }
  if (const char *value = std::getenv("RANK_4_EXCHANGE_MAX_DEPTH")) {
    candidate_max_depth = static_cast<std::uint32_t>(std::atoi(value));
  }
  if (const char *value =
          std::getenv("RANK_4_EXCHANGE_REFERENCE_MAX_DEPTH")) {
  ...
  try {
    ...
    std::cout << "summary games=" << games
              ...
              << summary.reference_residual_evaluations
              << " rebound=" << summary.candidate_rebound_probes << '/'
              << summary.candidate_rebound_wins << '/'
              << summary.candidate_rebound_losses
              << " exchange_ply1="
              << summary.candidate_exchange_ply1_probes << '/'
              << summary.candidate_exchange_ply1_cutoffs
              << " exchange_ply2="
