...
#define PAPER_SOCCER_EXCHANGE_PROOF_TESTING
#include "bot.cpp"
#undef PAPER_SOCCER_EXCHANGE_PROOF_TESTING
...
ps::GameState root_child_loss_choice(ps::Player mover) {
  ...
  const ps::Point root{4, 6};
  const ps::Point winning_reply_trap{player_one ? 5 : 3, 6 + forward};
  const ps::Point quiet{player_one ? 3 : 5, 6 + forward};
  const ps::Point closed_rebound{
      winning_reply_trap.x, winning_reply_trap.y + forward};

  ps::GameState state = make_clean_state_at(root, mover);
  state.visit_count[closed_rebound] = 1;
  block_edges_except(state, root, {winning_reply_trap, quiet});
  block_edges_except(state, winning_reply_trap, {root, closed_rebound});
  block_edges_except(state, closed_rebound, {winning_reply_trap});
  return state;
}
...
ps::GameState two_ply_exchange_component(ps::Player mover, bool win) {
  ...
  if (win) {
    ...
    for (int y = counterturn.y + forward; y != goal_y; y += forward) {
      const ps::Point current{4, y};
      state.visit_count[current] = 1;
      block_edges_except(state, current,
                         {previous, {4, y + forward}});
      previous = current;
    }
    block_edges_except(
        state, counterturn,
    ...
    const ps::Point closed_rebound{
        counterturn.x, counterturn.y + forward};
    state.visit_count[closed_rebound] = 1;
...
FixedSearchResult run_fixed_search(const ps::GameState &state, Config config) {
  ...
  FixedSearchResult result{action,
  ...
  if constexpr (requires { stats.exchange_ply1_probes; }) {
    result.exchange_ply1_probes = stats.exchange_ply1_probes;
    result.exchange_ply1_win_hits = stats.exchange_ply1_win_hits;
    result.exchange_ply1_loss_hits = stats.exchange_ply1_loss_hits;
    result.exchange_ply1_cutoffs = stats.exchange_ply1_cutoffs;
    result.exchange_ply2_probes = stats.exchange_ply2_probes;
    result.exchange_ply2_win_hits = stats.exchange_ply2_win_hits;
    result.exchange_ply2_loss_hits = stats.exchange_ply2_loss_hits;
    result.exchange_ply2_cutoffs = stats.exchange_ply2_cutoffs;
  }
  return result;
}

FixedSearchResult run_candidate_at_fixed_depth(
    const ps::GameState &state, cg::SearchConfig config,
    std::uint32_t depth) {
  const ps::GameState before = state;
  cg::CompleteTurnSearch search(state, config);
  const cg::RootResult result = search.run_fixed_depth_for_test(depth);
  require(same_state(state, before),
          "A fixed-depth reply proof must not mutate its input.");
...
void combined_time_and_node_budget_honors_the_safety_cap() {
  const ps::GameState state = ps::make_initial_state(codingame_rules());
  const ps::GameState before = state;
  cg::SearchConfig config;
  config.max_nodes = 32;
  config.max_time_ms = 1'000;
  cg::CompleteTurnSearch search(state, config);
  const std::vector<ps::Move> moves = search.run();

  require(search.stats().budget_exhausted && search.stats().nodes == 32,
          "A time-enabled search must still stop at its node safety cap.");
  require(!moves.empty() && same_state(state, before),
          "The capped time-mode search must return without mutating input.");
}
...
void root_child_win_proof_matches_leaf_reference() {
  for (const ps::Player mover : {ps::Player::One, ps::Player::Two}) {
    ...
    const FixedSearchResult fixed_candidate =
        run_candidate_at_fixed_depth(state, candidate_config, 2);
    ...
    const FixedSearchResult fixed_no_table =
        run_candidate_at_fixed_depth(state, no_table_config, 2);
    ...
    require(candidate.action == expected_action &&
                ...
                fixed_candidate.action == leaf_reference.action &&
                fixed_candidate.score == leaf_reference.score,
            "The root-child Win proof must preserve the fixed-depth action and score.");
    require(candidate.transposition_hits > 0 &&
                ...
                fixed_no_table.action == fixed_candidate.action &&
                fixed_no_table.score == fixed_candidate.score,
            "TT-first reply proof placement must preserve the no-table result.");
    require(candidate.nodes < leaf_reference.nodes &&
                fixed_candidate.goal_hits > 0 &&
                fixed_candidate.exchange_ply1_win_hits > 0 &&
                fixed_candidate.exchange_ply1_cutoffs ==
                    fixed_candidate.exchange_ply1_win_hits +
                        fixed_candidate.exchange_ply1_loss_hits,
            "The positive-depth Win proof should prune the losing reply branch.");
  }
}

void root_child_loss_proof_is_exact_and_symmetric() {
  for (const ps::Player mover : {ps::Player::One, ps::Player::Two}) {
    const ps::GameState state = root_child_loss_choice(mover);
    ...
    config.max_time_ms = 0;
    config.root_seed_endpoints = false;
    const FixedSearchResult with_table =
        run_candidate_at_fixed_depth(state, config, 2);

    config.transposition_entries = 0;
    const FixedSearchResult without_table =
        run_candidate_at_fixed_depth(state, config, 2);
    ...
    require(with_table.completed_depth == 2 &&
                with_table.action == expected_action &&
                with_table.score == expected_score &&
                with_table.loss_hits > 0 &&
                with_table.exchange_ply1_loss_hits > 0 &&
                with_table.exchange_ply1_cutoffs ==
                    with_table.exchange_ply1_win_hits +
                        with_table.exchange_ply1_loss_hits,
            "The first opponent boundary should prove its closed-component loss.");
    require(without_table.completed_depth == with_table.completed_depth &&
                without_table.action == with_table.action &&
                without_table.score == with_table.score &&
                without_table.loss_hits > 0,
            "The positive-depth Loss proof must be exact with the TT disabled.");

    ps::GameState reply = state;
    cg::apply_encoded_turn(reply, expected_action);
    require(reply.to_move == ps::opponent(mover),
            "The winning root action must hand the turn to the opponent.");
    cg::apply_encoded_turn(reply, mover == ps::Player::One ? "0" : "4");
    require(ps::winner(reply) == mover,
            "The proven reply component must really block and defeat the opponent.");
  }
}

void second_exchange_proofs_are_exact_and_symmetric() {
  for (const ps::Player mover : {ps::Player::One, ps::Player::Two}) {
    for (const bool win : {true, false}) {
      const ps::GameState state = two_ply_exchange_component(mover, win);
      ...
      config.root_seed_endpoints = false;
      const FixedSearchResult with_table =
          run_candidate_at_fixed_depth(state, config, 3);
      ...
      reference::SearchConfig reference_config;
      reference_config.max_turn_depth = 3;
      reference_config.max_nodes = 1'000'000;
      reference_config.max_time_ms = 0;
      reference_config.root_seed_endpoints = false;
      const FixedSearchResult leaf_reference =
      ...
      const std::string expected_action =
          mover == ps::Player::One ? "0" : "4";
      const int expected_score =
          (mover == ps::Player::One) == win ? 999'997 : -999'997;
      require(with_table.completed_depth == 3 &&
                  with_table.action == expected_action &&
                  with_table.score == expected_score &&
                  with_table.action == leaf_reference.action &&
                  with_table.score == leaf_reference.score,
              "A ply-two proof must preserve the leaf reference mate result.");
      require(without_table.completed_depth == with_table.completed_depth &&
                  without_table.action == with_table.action &&
                  without_table.score == with_table.score,
              "A ply-two proof must be exact with the TT disabled.");
      require(with_table.exchange_ply1_probes > 0 &&
                  with_table.exchange_ply1_cutoffs == 0 &&
                  with_table.exchange_ply2_probes > 0 &&
                  with_table.exchange_ply2_cutoffs ==
                      with_table.exchange_ply2_win_hits +
                          with_table.exchange_ply2_loss_hits,
              "Only the second exchange boundary should prove this fixture.");
      require(win ? with_table.exchange_ply2_win_hits > 0
                  : with_table.exchange_ply2_loss_hits > 0,
              "The ply-two diagnostic must classify the exact outcome.");

      ps::GameState counterturn = state;
      cg::apply_encoded_turn(counterturn, expected_action);
      cg::apply_encoded_turn(counterturn, expected_action);
      require(counterturn.to_move == mover,
              "Two fresh landings should return the turn to the root mover.");
      if (win) {
        const auto [action, stats] = search_with_one_node(counterturn);
        cg::apply_encoded_turn(counterturn, action);
        require(ps::winner(counterturn) == mover &&
                    stats.rebound_goal_hits > 0,
                "The proven ply-two Win must replay to the mover's goal.");
      } else {
        cg::apply_encoded_turn(counterturn, expected_action);
        require(ps::winner(counterturn) == ps::opponent(mover),
                "The proven ply-two Loss must block and defeat the mover.");
      }
    }
}
...
int main() {
  ...
  const std::vector<TestCase> tests{
      ...
      {"fixed_node_search_is_deterministic",
       fixed_node_search_is_deterministic},
      {"combined_time_and_node_budget_honors_the_safety_cap",
       combined_time_and_node_budget_honors_the_safety_cap},
      {"rebound_component_proofs_are_exact_and_symmetric",
       rebound_component_proofs_are_exact_and_symmetric},
      {"rebound_goal_is_proven_at_depth_zero",
       rebound_goal_is_proven_at_depth_zero},
      {"root_child_win_proof_matches_leaf_reference",
       root_child_win_proof_matches_leaf_reference},
      {"root_child_loss_proof_is_exact_and_symmetric",
       root_child_loss_proof_is_exact_and_symmetric},
      {"second_exchange_proofs_are_exact_and_symmetric",
       second_exchange_proofs_are_exact_and_symmetric},
