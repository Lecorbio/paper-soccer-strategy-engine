...
const outputPath = path.join(directory, "observed_arena_loss_regressions.hpp");
const checkOnly = process.argv[2] === "--check";
if (process.argv.length > 3 ||
    (process.argv.length === 3 && !checkOnly)) {
  throw new Error("Usage: node generate_observed_arena_loss_header.mjs [--check]");
}
...
if (checkOnly) {
  if (!fs.existsSync(outputPath) || fs.readFileSync(outputPath, "utf8") !== output) {
    throw new Error("observed_arena_loss_regressions.hpp is stale; regenerate it");
  }
  process.stdout.write(`Observed arena-loss regressions are current (${cases.length} cases).\n`);
} else {
  fs.writeFileSync(outputPath, output, "utf8");
  process.stdout.write(`Wrote observed_arena_loss_regressions.hpp (${cases.length} cases).\n`);
}
