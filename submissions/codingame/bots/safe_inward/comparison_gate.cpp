...
namespace papersoccer::safe_inward_engine {
namespace replay_value_model =
    ::papersoccer::rank5_engine::replay_value_model;
}
#endif

#define turn_action_v2 safe_inward_engine
#include "bot.cpp"
#undef turn_action_v2
...
namespace candidate = papersoccer::safe_inward_engine;
...
struct Decision {
  ...
  int root_score{};
  double milliseconds{};
  std::uint64_t safe_inward_shell_probes{};
  std::uint64_t safe_inward_rebound_activations{};
  std::uint64_t safe_inward_rebound_moves{};
};
...
Decision choose_candidate(ps::GameState &state, std::string_view transcript,
  ...
  if (!candidate::try_replay_correction(
    ...
    decision.completed_depth = search.stats().completed_turn_depth;
    decision.root_score = search.stats().root_score;
    decision.safe_inward_shell_probes =
        search.stats().safe_inward_shell_probes;
    decision.safe_inward_rebound_activations =
        search.stats().safe_inward_rebound_activations;
    decision.safe_inward_rebound_moves =
        search.stats().safe_inward_rebound_moves;
  }
  verify_complete_action(before, state, decision.action);
  return decision;
}
...
GameResult play(const Opening &opening, int candidate_player,
  ...
  while (!ps::is_terminal(state) && result.turns < maximum_turns) {
    ...
    const Decision decision = candidate_turn
        ? choose_candidate(state, transcript, node_budget)
        : choose_incumbent(state, transcript, node_budget);
    ...
    if (candidate_turn) {
      ...
      result.safe_inward_shell_probes += decision.safe_inward_shell_probes;
      result.safe_inward_rebound_activations +=
          decision.safe_inward_rebound_activations;
      result.safe_inward_rebound_moves += decision.safe_inward_rebound_moves;
    } else {
...
PairResult play_pair(const Opening &opening, const Options &options) {
  ...
  result.games[0] = play(opening, 0, options.node_budget, options.maximum_turns);
  result.games[1] = play(opening, 1, options.node_budget, options.maximum_turns);
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--help") {
      std::cout << "usage: comparison_gate --bank FILE --stage NAME "
                   ...
                   "--shard-index N --output FILE [identity hashes]\n";
      std::exit(0);
    }
    if (index + 1 >= argc) {
...
void write_game(std::ostream &out, const GameResult &game) {
  ...
  out << ",\"turns\":" << game.turns
      ...
      << ",\"safe_inward_shell_probes\":" << game.safe_inward_shell_probes
      << ",\"safe_inward_rebound_activations\":"
      << game.safe_inward_rebound_activations
      << ",\"safe_inward_rebound_moves\":"
      << game.safe_inward_rebound_moves << '}';
}
