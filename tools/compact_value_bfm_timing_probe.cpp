// Attribution-only derivative probe. Use the original engine probe for every
// throughput/p95 gate. Fixed configuration and trace match that original probe;
// the exclusive timer interval includes emergency action, search and encoding,
// and stops before all legality and actual-model full/delta checks.
...
int main(int argc,char **argv) {
  try {
    if (argc!=3) throw std::invalid_argument("usage: timing-probe ROOTS_TSV fixed");
    const std::string mode=argv[2];
    if (mode!="fixed") throw std::invalid_argument("attribution requires fixed work; clocked runs are forbidden");
    ...
    while (std::getline(input,line)) {
      ...
      rows.push_back({line.substr(0,tab),encoded,trace(result),ms,result.stats.generated_children,
          result.stats.evaluated_children,result.stats.tree_nodes,result.stats.expansions,timing});
    }
    std::cout << std::setprecision(17)
      << "{\"schema\":\"papersoccer.compact-engine-category-probe.v2\",\"mode\":\"" << mode
      << "\",\"source_sha256\":\"" << cvbfm_timing_v2::source_sha256
      << "\",\"instrumentation_version\":\"" << cvbfm_timing_v2::instrumentation_version
      << "\",\"attribution_only\":true,\"eligible_for_speed_gate\":false,\"payload_sha256\":\"" << cv::deployment_model().payload_sha256()
    ...
    for (std::size_t i=0;i<rows.size();++i) {
      ...
      std::cout << "{\"id\":\"" << r.id << "\",\"action\":\"" << r.action
        ...
        << "\",\"total_search_ns\":" << r.timing.total_search_ns
        << ",\"category_sum_ns\":" << r.timing.category_sum_ns
        << ",\"reconciled\":true,\"category_exclusive_ns\":{";
      for (std::size_t category=0;category<cvbfm_timing_v2::count;++category) {
        if (category) std::cout << ',';
        std::cout << '\"' << cvbfm_timing_v2::names[category] << "\":" << r.timing.ns[category];
      }
      std::cout << "},\"category_calls\":{";
      for (std::size_t category=0;category<cvbfm_timing_v2::count;++category) {
        if (category) std::cout << ',';
        std::cout << '\"' << cvbfm_timing_v2::names[category] << "\":" << r.timing.calls[category];
      }
      std::cout << "}}";
    }
    std::cout << "]}\n";return 0;
