...
#include "complete_turn_search_internal.hpp"
#include "papersoccer/rules.hpp"

// This is the only engine translation unit which includes the maintained
// ranked source. The macro removes only its protocol main; the search itself
// is consumed through the opaque adapter below without copying or modifying
// any rank_5 source file.
...
void copy_search_stats(const rank5::SearchStats &source,
                       CompleteTurnSearchStats &destination) noexcept {
...
void validate_complete_turn_action(const GameState &state,
  ...
  if (action.empty()) {
    throw std::logic_error("complete-turn search returned an empty action");
  }
  ...
  for (const Move move : action) {
    if (is_terminal(replayed) || replayed.to_move != mover) {
      throw std::logic_error(
          "complete-turn search returned an overlong action");
    }
    const std::vector<Move> legal = legal_moves(replayed);
    if (std::find(legal.begin(), legal.end(), move) == legal.end()) {
      throw std::logic_error("complete-turn search returned an illegal action");
    }
    replayed = apply_move(replayed, move);
  }
  if (!is_terminal(replayed) && replayed.to_move == mover) {
    throw std::logic_error(
        "complete-turn search returned an incomplete rebound action");
  }
}

CompleteTurnSearchResult run_complete_turn_search(
    const GameState &state, const CompleteTurnAnalysisConfig &config) {
  if (!supports_complete_turn_search(state.config)) {
    throw std::invalid_argument(
        "complete-turn search requires the standard 8x10 demo rules");
  }
  if (is_terminal(state)) {
    throw std::invalid_argument("cannot search a terminal game state");
  }
  ...
  search_config.max_turn_depth = config.max_turn_depth;
  search_config.max_nodes = config.max_nodes;
  search_config.transposition_entries = config.transposition_table_entries;
  search_config.evaluation_entries = config.evaluation_table_entries;
  search_config.max_time_ms = 0;
  search_config.absolute_deadline.reset();
  search_config.replay_value_blend_percent = 0;

  rank5::CompleteTurnSearch search(state, search_config);
  CompleteTurnSearchResult result;
  result.action = search.run();
  validate_complete_turn_action(state, result.action);
  copy_search_stats(search.stats(), result.stats);
  return result;
}

}  // namespace detail

Rank5DerivedBot::Rank5DerivedBot() = default;
...
const CompleteTurnAnalysisConfig &Rank5DerivedBot::config() const noexcept {
  return config_;
}
